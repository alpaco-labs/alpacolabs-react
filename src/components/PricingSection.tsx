        ...
        <div className="container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            animationDelay: `${index * 0.08}s`
                ...
                {/* Tooltip icon */}
                <Tooltip delayDuration={0} onOpenChange={open => setActiveTooltip(open ? pkg.id : null)}>
                  <TooltipTrigger asChild>
                    <button className="absolute top-4 right-4 text-muted-foreground hover:text-foreground transition-colors z-10">
                      <Info size={16} />
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="bottom" align="end" sideOffset={10} className="max-w-[250px] text-sm z-50 animate-fade-in">
                    {t(pkg.tooltipKey)}
                  </TooltipContent>
                </Tooltip>
